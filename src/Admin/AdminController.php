...
class AdminController {
    ...
    function pluginRowMeta($plugin_meta, $plugin_file, $plugin_data, $status){
        if($plugin_file === 'download-manager/download-manager.php') {
            $plugin_meta[] = "<strong><a href='https://www.wpdownloadmanager.com/wordpress-download-manager-change-log/' target='_blank'>" . __("Changelog", "download-manager") . "</a></strong>";
    ...
    {
        $menus  = apply_filters("wpdm_admin_page_header_menus/".wpdm_query_var('page'), $menus);
        $actions  = apply_filters("wpdm_admin_page_header_actions/".wpdm_query_var('page'), $actions);
    ...
    function metaBoxes() {
        ...
        $meta_boxes = array(
            ...
            'wpdm-attached-dir' => array('title' => __( "Attach Dir" , "download-manager" ), 'callback' => [$ServerDirBrowser, 'metaBox'], 'position' => 'side', 'priority' => 'core'),
            ...
            'wpdm-additional-previews' => array( "title" => "Additional Previews", "callback" => array($this, 'additionalPreviews'), "position" => "side", "priority" => "core" ),
        );
