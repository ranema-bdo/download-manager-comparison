<div id="lock-options"  class="tab-pane">
    ...
    <div class="wpdm-accordion w3eden">
        ...
        <div id="email" class="frm fwpdmlock panel-body"  <?php if(get_post_meta($post->ID,'__wpdm_email_lock', true)!='1') echo "style='display:none'"; ?> >
            ...
            <?php
            $custom_form_fields = get_post_meta($post->ID, '__wpdm_custom_form_field', true);
            $idl = (int)get_post_meta($post->ID, '__wpdm_email_lock_idl', true);
            $idle = get_post_meta($post->ID, '__wpdm_email_lock_idl_email', true);
            if (!$custom_form_fields) $custom_form_fields = array();

            ?>
            <div class="form-group">
                <label>
                    <input type="checkbox" name="file[custom_form_field][name]" value="1" <?php if ((int)wpdm_valueof($custom_form_fields, 'name') === 1) echo 'checked=checked'; ?> >
                    <?php _e("Ask for Visitor's Name", "download-manager"); ?>
                </label> <br/>
            </div>
            <fieldset class="form-group">
                ...
                <div class="form-group"><label><input type="radio" id="idl1" name="file[email_lock_idl]"
                                                      value="0" <?php if ($idl === 0) echo 'checked=checked'; ?>> <?php echo __("Mail Download Link", "download-manager"); ?>
                    </label></div>
                <div class="form-group"><label><input type="radio" id="idl2" name="file[email_lock_idl]"
                                                      value="2" <?php if ($idl === 2) echo 'checked=checked'; ?>> <?php echo __("Wait For Approval", "download-manager"); ?>
                    </label></div>

                <div class="form-group"><label id="email_lock_idl_lbl" <?php if ($idl == 1) echo 'class="dnedl"'; ?>><input
                                type="radio" id="idl" name="file[email_lock_idl]"
                                value="1" <?php if ($idl === 1) echo 'checked=checked'; ?> > <?php echo __("Download Instantly", "download-manager"); ?>
                    </label>
                    <label id="email_lock_idl_email_lbl"> & <input type="checkbox" id="idle" name="file[email_lock_idl_email]"
                                                                   value="1" <?php if ($idle == 1) echo 'checked=checked'; ?> > <?php echo __("Do Not Mail Download Link", "download-manager"); ?>
                    </label></div>
                <?php do_action("after_submit_form_action"); ?>
            </fieldset>

            <style>
                #email_lock_idl_lbl + #email_lock_idl_email_lbl {
                    display: none;
                }

                #email_lock_idl_lbl.dnedl + #email_lock_idl_email_lbl {
