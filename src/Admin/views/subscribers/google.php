...
?>
<form id="subsform" method="post" action="edit.php?post_type=wpdmpro&page=wpdm-subscribers&task=delete&lockOption=google" class="panel-body">

    <?php wp_nonce_field(WPDM_PRI_NONCE, '__wpdmdemail'); ?>
    ...
    <table id="subtbl" cellspacing="0" class="table table-striped">
        ...
        <?php foreach($res as $row) {
            ...
            <tr valign="top" class="author-self status-inherit" id="post-<?php echo $row['id']; ?>">
                ...
                <td class="author column-author"><?php echo wp_date("Y-m-d H:i",$row['timestamp']); ?></td>
                <td class="author column-author">
                    <?php
                    if ($row['processed']==0) echo "<a href='".admin_url("?export_google_contacts=".$row['ID'])."' class='btn btn-xs btn-secondary'>Export Contacts</a>";
