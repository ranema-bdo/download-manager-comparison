...
class Updater
{
    function __construct()
    {
        ...
        //add_filter( 'site_transient_update_plugins', [$this, 'updateAddon'] );
        //add_filter( 'transient_update_plugins', [$this, 'updateAddon'] );
...
NOTICE;
    ...
    function updateAddon($update_plugins){

        $upcheck = get_option('__wpdm_addon_update_check', false);
        //Check for update once a day
        if (($upcheck && (time() - $upcheck) < 86400)) return $update_plugins;
        ...
        $wpdm_addons = apply_filters("wpdm_addon_update", []);

        foreach ($wpdm_addons as $wpdm_addon) {
            $addondir = $wpdm_addon['dir'];
            $addon_latest = is_array($latest) && isset($latest[$addondir]) ? $latest[$addondir] : $wpdm_addon['Version'];

            //No update is available yet
            if (version_compare($wpdm_addon['Version'], $addon_latest, '<')) {
                $download_url = $wpdm_addon['download_url'];
                $update_plugins->response[$wpdm_addon['File']] = (object)array(
                    'slug' => $addondir,
                    'plugin' => $wpdm_addon['File'],
                    'new_version' => $addon_latest,
                    'url' => $wpdm_addon['PluginURI'],
                    'package' => $download_url,
                );
            }
        }
        update_option('__wpdm_addon_update_check', time(), false);
        return $update_plugins;
