...
class ServerDirBrowser {
    ...
    function dirGrid(){
            ...
            if( count($files) > 2 ) { /* The 2 accounts for . and .. */
                ...
                foreach( $files as $file ) {
                    if( $file != '.' && $file != '..' && is_dir($current_path . $file) ) {
                        ...
                        echo "<div class=\"col-md-4\"><a class='dir txtellipsis' onclick=\"explore('". $current_dir .'/'. $file ."', this)\" class=\"odir\" href=\"#\" rel=\"" . Crypt::encrypt($abs_dir_path) . "\"><i class=\"fa fa-folder-open\"></i> &nbsp;" . htmlentities($file) . "</a></div>";
                    }
                }
            }
        }
        ...
        foreach ($dirp as $dir){
            ...
            $breadcrumb .= "<li><a href='#' onclick=\"return explore('{$proot}/{$dir}')\" rel='".Crypt::encrypt($abs_path_current)."'>".$name."</a></li>";
            if($dir !== '')
                $proot = $proot.'/'.$dir;
        }
            ...
            <div class="dirs" style="padding: 15px">
                <div class="row" id="dirs-body">
                    ...
                    foreach ($dirs as $dir) {
                        ...
                        <div class="col-md-4">
                            <a class="dir txtellipsis" href="#" onclick="return explore('<?php echo $current_dir; ?>', this)" rel="<?php echo Crypt::encrypt($dir); ?>">
    ...
    function metaBox($post){
        ...
        $rel_path = Crypt::decrypt($dir);
        $rel_path = str_replace(AssetManager::root(), "", $rel_path);
        ?>
    ...
    /*
   function getFiles($dir, $recur = true){
       $dir = rtrim($dir,"/")."/";
       ...
       foreach($tmpfiles as $file){
           if( is_dir($dir.$file) && $recur == true) $files = array_merge($files,$this->getFiles($dir.$file, true));
           else
               $files[Crypt::Encrypt($dir.$file)] = $dir.$file;
       }
       return $files;
...
$ServerDirBrowser = new ServerDirBrowser();
