            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    if ($swl) {
                        ...
                        $mp3 = end($mp3);
                        $mp3 = strtolower($mp3);
                        $play = in_array($mp3, array('mp3')) ? "<a rel='nofollow' style='width: 32px' class='inddl btn btn-success btn-sm wpdm-btn-play song-{$ID}-{$pc} ml-2' data-song-index='song-{$ID}-{$pc}' id='song-{$ID}-{$pc}' data-state='stop' href='#' data-player='audio-player-{$ID}' data-title='{$fileTitle}' data-song='" . $download_url . "&forceplay=1&ind=" . $ind . "'><i style='margin:0' class='fa fa-play'></i></a>" : "";
                        if($play)
                            $play = apply_filters("wpdm_audio_play_button", $play, $download_url . "&forceplay=1&ind=" . $ind, $package, $fileID);
