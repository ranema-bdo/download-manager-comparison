...
class PackageController extends PackageTemplate
{
    ...
    function prepare($ID = null, $template = null, $template_type = 'page')
    {
        ...
        if (!isset($post_vars['files']) || !is_array($post_vars['files']))
            $post_vars['files'] = $this->getFiles($ID, true);

        if (in_array('file_count', $template_tags))
            $post_vars['file_count'] = count($post_vars['files']);


        if (in_array('file_list', $template_tags) || in_array('play_list', $template_tags) || in_array('audio_player', $template_tags)) {
            $post_vars['file_list'] = FileList::table($ID);
        ...
        if (in_array('file_list_extended', $template_tags))
        ...
        if (in_array('file_ext', $template_tags)) {
            $post_vars['files'] = isset($post_vars['files']) && is_array($post_vars['files']) ? $post_vars['files'] : $this->getFiles($ID);
        ...
        if (self::userDownloadLimitExceeded($post_vars['ID'])) {
            ...
            $post_vars['download_link_extended'] =
            $post_vars['download_link_popup'] = "<div class='alert alert-danger m-0 {$alert_size}' data-title='" . __("DOWNLOAD ERROR:", "download-manager") . "'>{$post_vars['link_label']}</div>";
        } //Not available yet
        else if (isset($post_vars['avail_date']) && $post_vars['avail_date'] != '' && strtotime($post_vars['avail_date']) > time()) {
            $post_vars['download_url'] = '#';
            $post_vars['link_label'] = __("Download will be available from ", "download-manager") . " " . date_i18n(get_option('date_format') . " h:i A", strtotime($post_vars['avail_date']));
            $post_vars['download_link'] =
            $post_vars['download_link_extended'] =
            $post_vars['download_link_popup'] = "<div class='alert alert-warning m-0 {$alert_size}' data-title='" . __("DOWNLOAD ERROR:", "download-manager") . "'>{$post_vars['link_label']}</div>";
        } //User is not allowed
        else if (is_user_logged_in() && !self::userCanAccess($post_vars['ID'])) {
            $post_vars['download_url'] = '#';
            $post_vars['link_label'] = Messages::permission_denied($ID);
            $post_vars['link_label'] = $post_vars['link_label'] !== '' ? $post_vars['link_label'] : __('You are not allowed to download.', 'download-manager');
            $post_vars['download_link'] =
            $post_vars['download_link_extended'] =
            $post_vars['download_link_popup'] = Messages::permission_denied();
            ...
            $post_vars['download_link_extended'] = UI::div(self::activeLocks($post_vars['ID'], array('embed' => 1)), "wpdm-lock-options", ["id" => "wpdm-lock-options"]);
     ...
     * @return bool
     */
    public function isLocked($package = null, $lock_option = '')
    {
        ...
        if (!is_array($package) && (int)$package > 0) {
            ...
            if($lock_option === '') {
                ...
                $package["{$lock_option}_lock"] = get_post_meta($id, "__wpdm_{$lock_option}_lock", true);
        } else
            $id = $package['ID'];
        $lock = '';
     ...
     * @return bool | string
     */
    function isPasswordProtected($ID = null)
    {
        $ID = $ID ?: $this->ID;
    ...
    /**
     * Get package or file passwords
     * @param null $ID
     * @param null $fileID
     * @return array|bool|float|int|mixed|string|string[]|null
     */
    function getPasswords($ID = null, $fileID = null)
    {
        $ID = $ID ?: $this->ID;
        $passwords = $this->isPasswordProtected($ID);
        if(!$passwords) return $passwords;
        $fileInfo = maybe_unserialize(get_post_meta($ID, '__wpdm_fileinfo', true));
        $fileInfo = $fileInfo ?: array();
        $filePasswords = __::valueof($fileInfo, "{$fileID}/password");
        $passwords = $filePasswords ?: $passwords;
        return $passwords;
    }
     ...
     * @return bool
     */
    public function userCanDownload($ID = null)
    {
        $ID = $ID ? : $this->ID;
     ...
     */
    public function size($ID = null, $recalculate = false)
    {
        $ID = $ID ?: $this->ID;
    ...
    public function productPreview($ID, $files = null, $width = 800, $height = 600)
    {
        ...
        switch ($type) {
            ...
            case 'pdf':
                ...
                $url .= "&ind={$ind}";
                $doc_preview_html = FileSystem::docViewer($url, $ID, $type);
    ...
    {
        global $wpdb, $wpdm_download_icon, $wpdm_download_lock_icon, $btnclass;
        $current_user = wp_get_current_user();
        if (is_array($extras))
            extract($extras);
        $data = '';
        ...
        if (is_user_logged_in() && count($matched) <= 0 && !@in_array('guest', @maybe_unserialize($package['access']))) {
            $package['download_url'] = "#";
            $package['download_link'] = $vars['download_link_extended'] = $vars['download_link_popup'] = Messages::permission_denied();
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
    ...
    /**
     * Get download button link label
     * @param null $ID
     * @param string $template_type
     * @return mixed|void
     */
    function getLinkLabel($ID = null, $template_type = 'link')
    {
        $link_label = null;
        if(!$ID) {
            $ID = $this->ID;
        ...
        $link_label = $link_label ?: get_post_meta($ID, '__wpdm_link_label', true);
    ...
    {
        global $wpdb, $wpdm_download_icon, $wpdm_download_lock_icon;
        if (is_array($extras))
            extract($extras);
        $data = '';
        $current_user = wp_get_current_user();
        ...
        $template_type = isset($template_type) ? $template_type : 'link';
        $link_label = $this->getLinkLabel($ID, $template_type);
        ...
        if ($expired !== "" && strtotime($expired) < time()) {
            $download_url = '#';
            $msg = sprintf(esc_attr__("Download was expired on %s", "download-manager"), date_i18n(get_option('date_format') . " h:i A", strtotime($expired)));
            $link_label = esc_attr__( 'Expired!', WPDM_TEXT_DOMAIN );
            return "<button type='button' title='{$msg}' onclick='alert(\"{$msg}\");' class='{$style} btn-danger ttip'>{$link_label}</div>";//$link_label = __("Download was expired on", "download-manager") . " " . date_i18n(get_option('date_format') . " h:i A", strtotime($expired));
        ...
        if ($publish !== "" && strtotime($publish) > time()) {
            $download_url = '#';
            $msg = sprintf(esc_attr__("Download will be available from %s", "download-manager"), date_i18n(get_option('date_format') . " h:i A", strtotime($publish)));
            return "<button type='button' title='{$msg}' onclick='alert(\"{$msg}\");' class='{$style} btn-secondary ttip'>{$link_label}</div>";
     ...
     * @return string
     */
    public function getDownloadURL($ID = null, $ext = array())
    {
        $ID = $ID ? : $this->ID;
        if ($this->isLocked($ID) && !Session::get('__wpdm_unlocked_' . $ID)) return '#locked';
        if (!is_array($ext)) $ext = [];
        $ext['wpdmdl'] = $ID;
        $ext['refresh'] = uniqid() . time();
        $permalink = get_permalink($ID);
        $permalink = apply_filters("wpdm_download_url_base", $permalink, $ID);
        if(isset($ext['filename'])) $ext['filename'] = esc_attr($ext['filename']);
    ...
    public function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        if (is_user_logged_in() && !self::userCanAccess($vars['ID']) && get_option('_wpdm_hide_all', 0) == 1) return $type != 'page' ? "" : Messages::permission_denied($vars['ID']);
        ...
        foreach ($flematches[0] as $nd => $scode) {
            $scode = str_replace(array("[", "]"), "", $scode);
            $vars[$scode] = FileList::extended($vars['ID'], $flematches[1][$nd], $flematches[2][$nd], $flematches[3][$nd]);
        }

        //Replace all image gallery tags
        foreach ($igematches[0] as $nd => $scode) {
            $scode = str_replace(array("[", "]"), "", $scode);
            $vars[$scode] = FileList::imageGallery($vars['ID'], $igematches[1][$nd], $igematches[2][$nd], $igematches[3][$nd]);
        }
        ...
        if ($vars['download_link'] == 'blocked' && $type == 'page') return Messages::permission_denied($vars['ID']);
    ...
    public function videoPreviewModal($package, $template_type = 'link')
    {
        ...
        $link_label = $link_label ? $link_label : $this->getLinkLabel($package['ID'], $template_type);
