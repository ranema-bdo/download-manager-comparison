...
$user = check_password_reset_key(__::query_var('key'), __::query_var('login'));
if(!is_wp_error($user)){
            ...
            $('#updatePassword').submit(function () {
                ...
                $(this).ajaxSubmit({
                    success: function (res) {
                        console.log(res);
                        if(res.success) {
                            $('#updatePassword').html("<div class='alert' data-title='<?php _e( "DONE!" , "download-manager" ); ?>'><b><?php _e( "Password Updated" , "download-manager" ); ?></b><br/><a style='margin-top:5px;padding:0.75rem 2rem;display:inline-block;' href='<?php echo wpdm_user_dashboard_url(); ?>'><?php _e( "Return to Login" , "download-manager" ); ?></a></div>");
                        } else
                            $('#updatePassword').html("<div class='alert alert-danger' data-title='<?php _e( "ERROR!" , "download-manager" ); ?>'><b><?php _e( "Password Update Failed" , "download-manager" ); ?></b><br/><a style='margin-top:5px;text-decoration:underline !important;' href='<?php echo WPDM()->user->login->lostPasswordURL(); ?>'>"+res.message+"</a></div>");
